...
export function Scanner({ onClose }) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        // Explicit prompt: getUserMedia surfaces the browser permission dialog
        // reliably within the user gesture from the Scan button click.
  ...
  const handleDecoded = async (token) => {
    ...
    try {
      const { data } = await api.post('/scan', { token, userId: user?.id });
      setResult(data);
      setStatus('success');
      if (data.type === 'pair' && data.points) {
        login({ ...user, total_points: (user?.total_points ?? 0) + data.points });
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 w-full h-full" style={{ backgroundColor: '#000' }}>
      ...
      {status === 'scanning' && (
        <>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-6 pointer-events-none">
            <h3 className="mb-6 text-center" style={{ color: '#fff' }}>Scan a QR Code</h3>
            ...
            <p className="text-center max-w-xs" style={{ color: '#E8E8E8', fontSize: '14px', lineHeight: 1.5 }}>
      ...
      {status === 'processing' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 gap-3">
          <div className="w-10 h-10 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--ocean-blue)', borderTopColor: 'transparent' }} />
      ...
      {status === 'success' && result && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85 px-6 text-center">
          {result.type === 'pair' ? (
            <>
              <p className="mb-1" style={{ color: '#E8E8E8', fontSize: '14px' }}>Paired with</p>
              <h2 className="mb-4" style={{ color: '#fff' }}>@{result.partner?.name}</h2>
              <h1 className="mb-2" style={{ color: 'var(--golden-amber)', fontSize: '40px', fontWeight: 500 }}>
                +{result.points} pts
              </h1>
              <p style={{ color: '#999', fontSize: '13px' }}>Both of you earned this</p>
              {result.completedChallenges?.length > 0 && (
                <div className="mt-4 space-y-1">
                  {result.completedChallenges.map((c) => (
                    <p key={c.id} style={{ color: 'var(--ocean-blue)', fontSize: '13px' }}>
                      Bonus: "{c.title}" +{c.points}
                    </p>
                  ))}
                </div>
              )}
            ...
            <>
              <h2 className="mb-2" style={{ color: '#fff' }}>{result.title || 'Challenge Found'}</h2>
              {result.description && (
                <p className="mb-4 max-w-xs" style={{ color: '#E8E8E8', fontSize: '14px', lineHeight: 1.5 }}>
                  {result.description}
                </p>
              )}
              {result.points > 0 && (
                <h1 style={{ color: 'var(--golden-amber)', fontSize: '36px', fontWeight: 500 }}>
                  +{result.points} pts available
                </h1>
              )}
            </>
          )}
